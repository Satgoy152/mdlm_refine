#!/bin/bash
#SBATCH --job-name=eval_qm9_guidance_mdlm_proseco
#SBATCH --output=slurm_output/eval_qm9_guidance_mdlm_proseco.out
#SBATCH --error=slurm_output/eval_qm9_guidance_mdlm_proseco.err
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# Submit with: sbatch --export=ALL,PROP=<qed|ring_count>[,GAMMA=<float>] eval_qm9_guidance_mdlm_proseco.sh
# Optional overrides:
...
#   PROSECO_BUDGET=<int>      (default: 1)    S: corrector passes
...
RUN_NAME=mdlm_${PROP}_proseco
DATA_CACHE_DIR=/nfs/turbo/coe-jjparkcv-medium/satyam/.cache
CKPT="${OUTPUT_DIR_BASE}/${RUN_NAME}"
RES="./eval_results"

results_csv_path="${RES}/qm9-eval-cfg_proseco_${PROP}_g-${GAMMA}_T-${SAMPLING_STEPS}_S-${PROSECO_BUDGET}_w-${PROSECO_FREQ}_seed-${SEED}.csv"
generated_seqs_path="${RES}/samples-qm9-eval-cfg_proseco_${PROP}_g-${GAMMA}_T-${SAMPLING_STEPS}_S-${PROSECO_BUDGET}_w-${PROSECO_FREQ}_seed-${SEED}.json"
