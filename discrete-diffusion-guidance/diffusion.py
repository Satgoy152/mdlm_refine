...
class Diffusion(L.LightningModule):
  ...
  # ============================================================
  # Refine / ProSeCo two-stage training helpers.
  # Pass 1 = standard MDLM forward.
  # Pass 2 = re-forward on a partially-corrected input where the
  # model's own pass-1 argmax fills non-MASK positions and a
  # configurable fraction of low-confidence positions are remasked.
  # Sampling between passes is non-differentiable (argmax), so
  # pass-1 and pass-2 grads are independent and can be summed.
  # ============================================================
  def _refine_get_loss_scales(self, loss_type):
    if loss_type == 'sum':
      return 1.0, 1.0
    if loss_type == 'mean':
      return 0.5, 0.5
    if loss_type == 'pass2_only':
      return 0.0, 1.0
    raise ValueError(f"Unknown refine_loss_type: {loss_type}")

  def _refine_get_remask_ratio(self, t, batch_size, device):
    if getattr(self.config.training, 'proseco', False):
      return torch.zeros(batch_size, device=device)
    cfg = getattr(self.config.training, 'refine_remask_ratio', 't')
    if cfg == 'random':
      return torch.rand(batch_size, device=device)
    if cfg == 't':
      # Continuous-time analogue of (t+1)/T: t itself
      # (= move_chance for log-linear noise).
      return t.detach()
    return torch.full((batch_size,), float(cfg), device=device)

  def _refine_add_gumbel_noise(self, log_probs, temperature):
    if temperature == 0:
      return log_probs
    u = torch.rand_like(log_probs).clamp_(min=1e-20, max=1.0 - 1e-7)
    gumbel = -torch.log(-torch.log(u))
    return log_probs + temperature * gumbel

  def _refine_remask_tokens(self, log_probs, sampled_tokens,
                            original_mask, remask_ratio):
    """Build the pass-2 input.

      - Every position is filled with the pass-1 argmax sample.
        (No source/target split for QM9; at positions that were
         already unmasked in xt, SUBS forces argmax == original
         token, so this is a no-op there.)
      - Among originally-masked positions, the lowest-confidence
        `remask_ratio` fraction is re-MASKed.
    """
    probs = log_probs.detach().exp()
    sampled_probs = torch.gather(
      probs, dim=-1, index=sampled_tokens.unsqueeze(-1)).squeeze(-1)

    # Only originally-masked positions are eligible for remasking.
    ...
    for b in range(sampled_tokens.shape[0]):
      num_masked = int(original_mask[b].sum().item())
      num_to_remask = int(remask_ratio[b].item() * num_masked)
      if num_to_remask == 0:
        continue
      _, lowest_conf_idx = torch.topk(
        confidence[b], k=num_to_remask, largest=False)
      result[b, lowest_conf_idx] = self.mask_index
    return result
  ...
  def _refine_pass2_loss(self, x0, xt, log_probs_pass1,
    ...
    remask_ratio = self._refine_get_remask_ratio(
    ...
    with torch.cuda.amp.autocast(dtype=torch.float32):
      logits_2 = self.backbone(xt2, sigma_processed, cond)

    return F.cross_entropy(
      logits_2.reshape(-1, logits_2.shape[-1]),
      x0.reshape(-1),
  ...
  def _forward_pass_diffusion(self, x0, cond=None):
    ...
    if self.diffusion == 'absorbing_state':
      ...
      pass1_per_token = - log_p_theta * (dsigma / torch.expm1(sigma))[:, None]

      # Refine / ProSeCo: training-only auxiliary pass 2.
      refine_enabled = self.training and (
          getattr(self.config.training, 'refine', False)
