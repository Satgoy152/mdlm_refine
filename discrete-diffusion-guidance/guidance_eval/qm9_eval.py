...
def main(config: omegaconf.DictConfig) -> None:
  ...
  guidance_cfg = getattr(config, 'guidance', None)
  guidance_items = list(guidance_cfg.items()) if guidance_cfg is not None else []
  ...
  } | {k.capitalize(): -1 for k, v in guidance_items})
  ...
  } | {k.capitalize(): v for k, v in guidance_items})
  print("Guidance:", ", ".join([f"{k.capitalize()} - {v}" for k, v in guidance_items]))
