...
#SBATCH --output=train_mdlm_refine_presco.out
#SBATCH --error=train_mdlm_refine_presco.err
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
...
python main.py \
  ...
  model.length=1024 \
  wandb.name=mdlm-owt-finetuned-wt103-r-0.5a-50k-presco \
  wandb.id="mdlm-owt-finetuned-wt103-r-0.5a-50k-presco_$(date +%Y%m%d_%H%M%S)" \
