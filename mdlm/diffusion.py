...
class Diffusion(L.LightningModule):
  ...
  def on_load_checkpoint(self, checkpoint):
    ...
    self.fast_forward_batches = checkpoint['loops'][
      'fit_loop']['epoch_loop.batch_progress'][
        'current']['completed']
    # # Handle key mismatch when checkpoint was saved with backbone=hf_dit
    # # (keys: backbone.backbone.xxx) but current model uses backbone=dit
    # # (keys: backbone.xxx). Strip the extra backbone. prefix.
    # state_dict = checkpoint['state_dict']
    # if (any(k.startswith('backbone.backbone.') for k in state_dict)
    #     and not any(k.startswith('backbone.backbone.')
    #                 for k in self.state_dict())):
    #   new_state_dict = {}
    #   for k, v in state_dict.items():
    #     if k.startswith('backbone.backbone.'):
    #       new_state_dict['backbone.' + k[len('backbone.backbone.'):]] = v
    #     else:
    #       new_state_dict[k] = v
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None):
    ...
    if p_x0 is None:
      if self.config.training.refine:
        # Bypass SUBS identity constraint so unmasked positions can change.
        # self.forward() forces p_x0 to be one-hot at the current token for
        # unmasked positions (via _subs_parameterization), which would prevent
        # any refinement. Instead, call the backbone directly and only suppress
        # the mask token, leaving all real-token logits free.
        sigma = self._process_sigma(sigma_t)
        with torch.cuda.amp.autocast(dtype=torch.float32):
          logits = self.backbone(x, sigma)
        logits[:, :, self.mask_index] += self.neg_infinity
    ...
    if self.config.training.refine:
      # All positions (masked and unmasked) are free to change.
      return p_x0, _x
  ...
  def _sample(self, num_steps=None, eps=1e-5):
    ...
    for i in range(num_steps):
      ...
      if self.sampler == 'ddpm':
        ...
        if (self.config.training.refine
            or not torch.allclose(x_next, x)
            or self.time_conditioning):
          # Disable caching. When refine=True every position can change
          # each step so the cache is always stale.
          p_x0_cache = None
        x = x_next
      else:
  ...
  def _combine_loss(self, loss_1, loss_2, loss_type = "alpha"):
    if loss_type == "alpha":
      return self.config.training.alpha * loss_1 + (1 - self.config.training.alpha) * loss_2
    else:
  ...
  def remask_tokens(self,logits, sampled_tokens, original_mask, remask_ratio, mask_token_id):
    ...
    for b in range(sampled_tokens.shape[0]):
        num_to_mask = int(remask_ratio[b].item() * original_mask[b].sum().item())
        if num_to_mask == 0:
            continue
